...
from torch.nn.init import orthogonal_, xavier_normal_

# Container to return all required values from model
PolicyReturn = namedlist.namedlist(
    "PolicyReturn",
    [
        ("latent_state", None),
        ("value_estimate", None),
...
class Policy(nn.Module):
    ...
    def encode(
        ...
        """
        raise NotImplementedError(
            "Should be provided by child class, e.g. RNNPolicy or DVRLPolicy."
        )
    ...
    def forward(
        self, current_memory, deterministic=False, predicted_times=None, savedir=None
    ):
        """
        ...
        latent_state_for_encoding = (
            latent_state.detach() if self.detach_encoder else latent_state
        )
        encoded_state = (
            self.encode_particles(latent_state_for_encoding)
...
class RNNPolicy(Policy):
    ...
    def encode(
        ...
        if self.obs_loss_coef > 0:
            o_dec = self.linear_obs_decoder(
                torch.cat([previous_latent_state, encoded_actions], dim=1)
            ).view(-1, *self.cnn_output_dimension)
            ...
            if predicted_times is not None:
                predicted_observations = self.predict_observations(
                    latent_state, encoded_actions, obs_predicted, predicted_times
                )

        # This is only used for DVRL
        predicted_particles = (
            None
            if predicted_observations is None
        ...
        return (
            latent_state,
            encoding_loss,
            (torch.tensor(0), predicted_obs_loss),
            0,
    ...
    def predict_observations(
        self, latent_state, encoded_actions, obs_predicted, predicted_times
    ):
        """
