...
def euler_q(pi, s, a, p_sum, n, r_sum, v_upper, v_lower, t, b_r):
    ...
    v_dist = np.sqrt(
        sum([p_hat[i] * (v_upper[i] - v_lower[i]) ** 2 for i in range(len(p_hat))])
    )
    b_pv = 1.0 / np.sqrt(n_s_a) * (JB / np.sqrt(n_s_a) + Bv * v_dist) + find_phi(
        p_hat, v_upper, n_s_a
    )
    pi_act = pi[(s, t)]
...
def euler_s(s, pi, n, p_sum, r_sum, r_var, v_upper, v_lower, t):
    ...
    pi_act = np.random.choice(
        np.argwhere(q == np.amax(q)).flatten()
    )  # randomly choose if have same q vals
    pi[(s, t)] = pi_act
    ...
    b_r_argmax = np.sqrt(2 * C * r_var_hat / n_s_a) + 7 * C / (
        3 * max(1, n[(s, pi_act)] - 1)
    )
...
def euler_iter(pi, n, p_sum, r_sum, r_var):
    ...
    for t in range(H, 0, -1):  # t from H~1
        ...
        for state in range(S):
            pi, _upper, _lower = euler_s(
                state, pi, n, p_sum, r_sum, r_var, v_upper, v_lower, t
            )
...
def explore_pi(
    pi, n, p_sum, r_sum, r_var, n_global, p_global, r_global, goal_s, goal_a
):
    # initialize with a fixed start state
...
def euler_explore(n_global, p_global, r_global, goal_s, goal_a):
    ...
    for k in range(N_euler):
        ...
        pi = euler_iter(pi, n, p_sum, r_sum, r_var)
        time.time()
        reward, n, p_sum, r_sum, r_var, n_global, p_global, r_global = explore_pi(
            pi, n, p_sum, r_sum, r_var, n_global, p_global, r_global, goal_s, goal_a
        )
...
def pomdp(goal, verbose=True):
    ...
    # for s in range(S):
    for a in range(A):
        n_global, p_global, r_global = euler_explore(
            n_global, p_global, r_global, goal, a
        )
    if verbose:
