...
def euler_vec(n_mat, p_sum, r_sum, r_var, v_upper, v_lower, t):
    ...
    phi = np.sqrt(np.sum(temp, axis=2) * 2 * C / n_denom) + H * C / (
        3 * denom
    )  # multiply second term by H
    ...
    # phi, S
    phi = np.sqrt(temp * 2 * C / n_denom) + H * C / (
        3 * n_temp
    )  # multiply second term by H
...
def euler_explore(n, p_sum, r_sum, r_var, n_global, p_global, r_global, s, a):
    ...
    n, p_sum, r_sum, r_var, n_global, p_global, r_global, rew, steps = explore_pi(
        pi, n, p_sum, r_sum, r_var, n_global, p_global, r_global, s, a
    )
...
def pomdp(save=True):
    ...
    while total_eps > 0:
            ...
            for a in range(A):
                ...
                while n_global[s, a] < MIN_VISITS and _t < MIN_VISITS:
                    (
                        ...
                        n, p_sum, r_sum, r_var, n_global, p_global, r_global, s, a
                    )
        ...
        if save:
            print(
                "Finished collecting all samples at least 10 times or reached max step"
            )
        ...
        n_global, p_global, r_global, rew, steps = random_explore(
            n_global, p_global, r_global
        )
        stats["rew"].append(rew)
        stats["steps"].append(steps)
        total_eps -= 1
