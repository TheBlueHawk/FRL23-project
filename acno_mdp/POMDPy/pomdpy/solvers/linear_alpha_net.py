...
from experiments.scripts.pickle_wrapper import load_pkl, save_pkl

from .alpha_vector import AlphaVector
from .base_tf_solver import BaseTFSolver
from .ops import clipped_error, select_action_tf, simple_linear
...
class LinearAlphaNet(BaseTFSolver):
    ...
    def train(self, epoch):
        start_step = self.step_assign_op.eval(
            {self.step_input: epoch * self.model.max_steps}
        )

        total_reward, avg_reward_per_step, total_loss, total_v, total_delta = (
            0.0,
            0.0,
            0.0,
            0.0,
        ...
        for step in range(start_step, start_step + self.model.max_steps):
            ...
            if step_result.is_terminal:
                v_b_next = np.array([0.0])
            else:
                next_belief = self.model.belief_update(
            ...
            avg_reward_per_step = total_reward / (step + 1.0)
            avg_loss = loss / (step + 1.0)
            avg_v = total_v / (step + 1.0)
            avg_delta = total_delta / (step + 1.0)
            ...
            self.inject_summary(
                {
                    ...
                    "training.weights": self.sess.run(
                        self.w["l1_w"],
                        feed_dict={
                            self.ops["l0_in"]: np.reshape(
                                self.model.get_reward_matrix().flatten(), [1, 6]
                            ),
    ...
    def e_greedy_predict(self, belief, epsilon_step):
        ...
        action, v_b, epsilon = self.sess.run(
            [self.ops["a"], self.ops["v_b"], self.ops["epsilon_op"]],
            feed_dict={
                self.ops["l0_in"]: np.reshape(
                    self.model.get_reward_matrix().flatten(), [1, 6]
                ),
    ...
    def greedy_predict(self, belief):
        # try hard-coding input of linear net to be rewards (can try random as well)
        action, v_b = self.sess.run(
            [self.ops["a"], self.ops["v_b"]],
            feed_dict={
                self.ops["l0_in"]: np.reshape(
                    self.model.get_reward_matrix().flatten(), [1, 6]
                ),
    ...
    def gradients(self, target_v, belief, learning_rate_step):
        return self.sess.run(
            [self.ops["optim"], self.ops["loss"], self.ops["delta"]],
            feed_dict={
    ...
    def build_linear_network(self):
        with tf.variable_scope("linear_fa_prediction"):
            self.ops["belief"] = tf.placeholder(
                "float32", [self.model.num_states], name="belief_input"
            )

            with tf.name_scope("linear_layer"):
                self.ops["l0_in"] = tf.placeholder(
                    "float32",
                    [1, self.model.num_states * self.model.num_actions],
                    name="input",
                )
                self.ops["l1_out"], self.w["l1_w"], self.w["l1_b"] = simple_linear(
                    self.ops["l0_in"], activation_fn=None, name="weights"
                )

                self.ops["l1_out"] = tf.reshape(
                    self.ops["l1_out"],
                    [self.model.num_actions, self.model.num_states],
                    name="output",
                )
            ...
            with tf.variable_scope("action_selection"):
                ...
                self.ops["a"], self.ops["v_b"] = select_action_tf(
                    self.ops["belief"], vector_set
                )

            with tf.variable_scope("epsilon_greedy"):
                self.ops["epsilon_step"] = tf.placeholder(
                    "int64", None, name="epsilon_step"
                )
                self.ops["epsilon_op"] = tf.maximum(
                    self.model.epsilon_minimum,
        ...
        with tf.variable_scope("linear_optimizer"):
            ...
            self.ops["loss"] = tf.reduce_mean(
                clipped_error(self.ops["delta"])
                + self.model.beta * tf.nn.l2_loss(self.w["l1_w"])
                + self.model.beta * tf.nn.l2_loss(self.w["l1_b"]),
                name="loss",
            )

            self.ops["learning_rate_step"] = tf.placeholder(
                "int64", None, name="learning_rate_step"
            )
            self.ops["learning_rate_op"] = tf.maximum(
                self.model.learning_rate_minimum,
                tf.train.exponential_decay(
            ...
            self.ops["optim"] = tf.train.MomentumOptimizer(
                self.ops["learning_rate_op"], momentum=0.8, name="Optimizer"
        ...
        with tf.variable_scope("linear_fa_summary"):
            ...
            for tag in scalar_summary_tags:
                ...
                self.summary_ops["{}".format(tag)] = tf.summary.scalar(
                    "{}".format(tag), self.summary_placeholders[tag]
                )

            self.summary_placeholders["training.weights"] = tf.placeholder(
                "float32", [1, 6], name="training_weights"
            )
            self.summary_ops["training.weights"] = tf.summary.histogram(
                "weights", self.summary_placeholders["training.weights"]
            )
            self.summary_ops["writer"] = tf.summary.FileWriter(
                self.model.logs, self.sess.graph
            )
    ...
    def inject_summary(self, tag_dict, step):
        summary_str_lists = self.sess.run(
            [self.summary_ops["{}".format(tag)] for tag in tag_dict.keys()],
            feed_dict={
    ...
    def save_alpha_vectors(self):
        ...
        save_pkl(
            av, os.path.join(self.model.weight_dir, "linear_alpha_net_vectors.pkl")
        )
