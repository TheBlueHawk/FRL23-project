...
class Agent:
    ...
    def discounted_return(self, evaln=False):
        ...
        if self.model.solver == "ValueIteration":
            ...
            if self.model.save:
                save_pkl(
                    solver.gamma,
                    os.path.join(
                        self.model.weight_dir,
                        "VI_planning_horizon_{}.pkl".format(
        ...
        console(
            2,
            module,
            "ave undiscounted return/step: "
            + str(self.experiment_results.undiscounted_return.mean)
            + " +- "
        ...
        console(
            2,
            module,
            "ave discounted return/step: "
            + str(self.experiment_results.discounted_return.mean)
            + " +- "
        ...
        self.logger.info(
            ...
            + "\t"
            + "ave undiscounted return: "
            + str(self.experiment_results.undiscounted_return.mean)
            + " +- "
            + str(self.experiment_results.undiscounted_return.std_err())
            + "\t"
            + "ave discounted return: "
            + str(self.experiment_results.discounted_return.mean)
            + " +- "
            + str(self.experiment_results.discounted_return.std_err())
            + "\t"
    ...
    def multi_epoch(self):
        ...
        for i in range(self.model.n_epochs):
            ...
            if i % 100 == 0:
                np.save(
                    open(DIR + "{}/epoch_{}_rewards.npy".format(SAVE_K, i), "wb"),
                    np.array(rewards),
                )
    ...
    def run_pomcp(self, epoch, eps, temp=None, observe_then_plan=True):
        ...
        if not observe_then_plan and (epoch % 100 == 0):
            ...
            np.save(
                open(DIR + "{}/epoch{}_N.npy".format(SAVE_K, epoch), "wb"),
                self.model.n_counts,
            )
        ...
        if observe_then_plan:
            ...
            for terminal in terminal_states:
                solver.model.t_estimates[terminal, :, :] = 0.0
                solver.model.t_estimates[terminal, :, terminal] = 1.0
                solver.model.r_estimates[terminal, :] = 0.0
        ...
        for i in range(self.model.max_steps):
            ...
            action = solver.select_eps_greedy_action(
                eps, start_time, greedy_select=(not _eval)
            )
            ...
            step_result, is_legal = self.model.generate_step(
                state, action, _true=True, is_mdp=mdp
            )
            start_time = time.time()
            discounted_reward += discount * (step_result.reward)
            reward += step_result.reward
            ...
            if (not observe_then_plan) and (not _eval):
                if past_obs != 720 and action.bin_number < 8:
                    ...
                    self.model.r_counts[
                        past_obs, action.bin_number % 8
                    ] += step_result.reward
                    self.model.t_counts[
                        past_obs,
                        action.bin_number % 8,
                        step_result.observation.position,
                    ] += 1
        ...
        console(
            3,
            module,
            "Total possible undiscounted return: "
    ...
    def run_value_iteration(self, solver, epoch, evaln=False):
        ...
        if evaln:
            ...
            save_pkl(
                solver.gamma,
                os.path.join(
                    "VI_planning_horizon_{}_adjusted.pkl".format(
                        self.model.planning_horizon
                    )
                ),
            )
