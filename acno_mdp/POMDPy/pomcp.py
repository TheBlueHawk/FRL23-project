...
from pomdpy.solvers import MCP, POMCP
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--solver", default="POMCP", type=str, help="Specify the solver to use {POMCP}"
    )
    parser.add_argument(
        "--seed",
        default=1993,
    ...
    parser.add_argument(
        "--use_tf", dest="use_tf", action="store_true", help="Set if using TensorFlow"
    )
    parser.add_argument(
        "--discount",
        default=0.7,
        type=float,
        help="Specify the discount factor (default=0.95)",
    )
    parser.add_argument(
        "--n_epochs",
        default=100,
    ...
    parser.add_argument(
        "--test", default=10, type=int, help="Evaluate the agent every `test` epochs"
    )
    ...
    parser.add_argument(
        ...
        default=3.0,
        type=float,
        help="Coefficient for UCB algorithm used by MCTS",
    )
    parser.add_argument(
        "--n_start_states",
        default=2000,
    ...
    parser.add_argument(
        "--action_selection_timeout",
        default=60,
        type=int,
        help="Max num of secs for action selection",
    )
    ...
    env = Sepsis(
        init_state_idx=args["init_idx"], cost=args["cost"], is_mdp=args["is_mdp"]
    )
