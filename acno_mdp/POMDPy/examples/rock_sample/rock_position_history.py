...
class RockData(object):
    ...
    def to_string(self):
        ...
        data_as_string = (
            ...
            + " Goodness number: "
            + str(self.goodness_number)
            + " Probability that rock is good: "
            + str(self.chance_good)
        )
...
class PositionAndRockData(HistoricalData):
    ...
    def deep_copy(self):
        ...
        return PositionAndRockData(
            self.model, self.grid_position.copy(), self.all_rock_data, self.solver
        )
    ...
    def shallow_copy(self):
        ...
        return PositionAndRockData(
            self.model, self.grid_position.copy(), new_rock_data, self.solver
        )
    ...
    def create_child(self, rock_action, rock_observation):
        ...
        next_position, is_legal = self.model.make_next_position(
            self.grid_position.copy(), rock_action.bin_number
        )
        ...
        if rock_action.bin_number is ActionType.SAMPLE:
            ...
            if np.abs(likelihood_good) < 0.01 and np.abs(likelihood_bad) < 0.01:
                ...
                rock_data.chance_good = old_div(
                    likelihood_good, (likelihood_good + likelihood_bad)
                )
    ...
    def generate_smart_actions(self):
        ...
        for i in range(0, n_rocks):
            rock_data = self.all_rock_data[i]
            if (
                rock_data.chance_good != 0.0
                and rock_data.chance_good != 1.0
                and np.abs(rock_data.goodness_number) < 2
            ):
