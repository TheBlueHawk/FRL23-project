...
import json
import logging
from builtins import hex, map, range, str
...
from pomdpy.util import config_parser, console

from .grid_position import GridPosition
from .rock_action import ActionType, RockAction
from .rock_observation import RockObservation
from .rock_position_history import PositionAndRockData, RockData
...
class RockModel(Model):
    ...
    def get_sensor_correctness_probability(self, distance):
        assert self.half_efficiency_distance is not 0, self.logger.warning(
            "Tried to divide by 0! Naughty naughty!"
        )
        return (
            1 + np.power(2.0, old_div(-distance, self.half_efficiency_distance))
        ) * 0.5
    ...
    def is_valid_state(self, rock_state):
        ...
        return (
            0 <= pos.i < self.n_rows
            and 0 <= pos.j < self.n_cols
            and self.get_cell_type(pos) is not RSCellType.OBSTACLE
        )
    ...
    def create_root_historical_data(self, solver):
        ...
        return PositionAndRockData(
            self, self.start_position.copy(), self.all_rock_data, solver
        )
    ...
    def make_next_state(self, state, action):
        ...
        next_position, is_legal = self.make_next_position(
            state.position.copy(), action_type
        )
    ...
    def make_observation(self, action, next_state):
        ...
        # if checking a rock...
        dist = next_state.position.euclidean_distance(
            self.rock_positions[action.rock_no]
        )
