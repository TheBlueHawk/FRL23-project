...
from pomdpy.pomdp import HistoricalData, Model, StepResult
...
class Sepsis:
    def __init__(self, init_state_idx, missing=True, cost=-0, is_mdp=0):
        ...
        self.sim = SepsisEnv(
            init_state=self.init_state, no_missingness=bool(is_mdp == 1)
        )
    ...
    def generate_particles(
        self, prev_belief, action, obs, n_particles, prev_particles, mdp
    ):
        particles = []
            ...
            if time.time() - start > self.particle_selection_timeout:
                if obs.position != 720:
                    print(
                        ...
                        "prob:",
                        self.t_estimates[
                            state.position, action.bin_number % 8, obs.position
                        ],
                    )
                    print(
                        "real pos{}, real transition:".format(self.real_state),
                        self.t_estimates[
                            self.real_state, action.bin_number, obs.position
                        ],
                    )
    ...
    def empirical_simulate(self, state, action):
        ...
        if debug_mode:
            if (state, action) in self.empi_model.keys():
                ...
                p /= sum(p)
                next_state = np.random.choice(720, 1, p=p)[
                    0
        ...
        temp.env.state.set_state_by_idx(
            int(next_state), idx_type="obs", diabetic_idx=int(0)
        )
    ...
    def generate_step(self, state, action, _true=False, is_mdp=False):
        ...
        result.observation = self.make_observation(
            action, result.next_state, always_obs=is_mdp
        )
        result.reward = self.make_reward(
            state, action, result.next_state, is_legal, always_obs=is_mdp
        )
