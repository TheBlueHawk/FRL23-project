...
from .tiger_action import ActionType, TigerAction
...
class TigerModel(model.Model):
    ...
    def sample_state_informed(self, belief):
        ...
        :return:
        """
        s = 100.0 * np.random.random()
        int1 = np.array([0.0, 100.0 * belief[0]])
        if int1[0] <= s <= int1[1]:
    ...
    def get_all_actions(self):
        ...
        """
        return [
            TigerAction(ActionType.LISTEN),
            TigerAction(ActionType.OPEN_DOOR_1),
    ...
    def get_reward_matrix():
        ...
        :return:
        """
        return np.array([[-1.0, -1.0], [-20.0, 10.0], [10.0, -20.0]])
    ...
    def belief_update(self, old_belief, action, observation):
        ...
        if observation.source_of_roar[0]:
            observation_probability = (probability_correct * p1_prior) + (
                probability_incorrect * p2_prior
            )
            p1_posterior = old_div(
                (probability_correct * p1_prior), observation_probability
            )
            p2_posterior = old_div(
                (probability_incorrect * p2_prior), observation_probability
            )
        # Observation 2 - the roar came from door 1
        else:
            observation_probability = (probability_incorrect * p1_prior) + (
                probability_correct * p2_prior
            )
