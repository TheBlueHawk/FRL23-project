...
class StateRandomVariable(RandomVariable):
    ...
    def __getitem__(self, key):
        ...
        raise KeyError(
            "StateRandomVariable only supports slicing through the method slice_elements()"
        )
    ...
    def __setattr__(self, name, value):
        ...
        elif ("_items" in self.__dict__) and (name in self__dict__["_items"]):
            raise AttributeError(
    ...
    def sample_reparameterized(self, batch_size, num_particles):
        ...
        for name, random_variable in self.named_random_variables():
            setattr(
                state,
                name,
                random_variable.sample_reparameterized(
                    batch_size=batch_size, num_particles=num_particles
                ),
            )
        return state
    ...
    def set_random_variable_(self, name, random_variable):
        if not isinstance(random_variable, RandomVariable):
            raise TypeError(
                "random_variable {} is not a RandomVariable".format(random_variable)
            )
        _items = self.__dict__["_items"]
    ...
    def _find_common_keys(self, other):
        ...
        if not set(common_keys) == set(random_variable_keys):
            logging.warning(
                "Not all random variable key are used, only {} out of {}!".format(
        ...
        if not set(common_keys) == set(other_keys):
            logging.debug(
                "Not all other keys are used/evaluated, only {} out of {}!".format(
...
class MultivariateIndependentLaplace(RandomVariable):
    ...
    def sample(self, batch_size, num_particles):
        ...
        if isinstance(self._location, Variable):
            ...
            return self._location.detach() - self._scale.detach() * torch.sign(
                uniforms
            ) * torch.log(1 - 2 * torch.abs(uniforms))
        else:
            return self._location - self._scale * torch.sign(uniforms) * torch.log(
                1 - 2 * torch.abs(uniforms)
            )
    ...
    def logpdf(self, value, batch_size, num_particles):
        ...
        return torch.sum(
            (
                -torch.abs(value - self._location) / self._scale
                - torch.log(2 * self._scale)
            ).view(batch_size, num_particles, -1),
            dim=2,
        )
...
class MultivariateIndependentNormal(RandomVariable):
    ...
    def sample(self, batch_size, num_particles):
        ...
        return self._mean.detach() + Variable(uniform_normals) * torch.sqrt(
            self._variance.detach()
        )
    ...
    def sample_reparameterized(self, batch_size, num_particles):
        ...
        return self._mean + torch.sqrt(self._variance) * standard_normal.sample(
            batch_size, num_particles
        )
    ...
    def pdf(self, value, batch_size, num_particles):
        ...
        return torch.prod(
            (
                1
                / torch.sqrt(2 * self._variance * np.pi)
                * torch.exp(-0.5 * (value - self._mean) ** 2 / self._variance)
            ).view(batch_size, num_particles, -1),
            dim=2,
        )
    ...
    def kl_divergence(self, other_random_variable):
        ...
        kld_element = (
            torch.log(torch.sqrt(var_2))
            - torch.log(torch.sqrt(var_1))
            + (var_1 + (mean_1 - mean_2).pow(2)) / (2 * var_2)
            - 0.5
        )
...
class MultivariateIndependentPseudobernoulli(RandomVariable):
    ...
    def sample(self, batch_size, num_particles):
        assert list(self._probability.size()[:2]) == [batch_size, num_particles]
    ...
    def pdf(self, value, batch_size, num_particles):
        ...
        return torch.prod(
            (self._probability**value * (1 - self._probability) ** (1 - value)).view(
...
class Laplace(RandomVariable):
    ...
    def __init__(self, location, scale):
        ...
        self._multivariate_independent_laplace = MultivariateIndependentLaplace(
            location=location.unsqueeze(-1), scale=scale.unsqueeze(-1)
        )

    def sample(self, batch_size, num_particles):
        return self._multivariate_independent_laplace.sample(batch_size, num_particles)
...
class Choice(RandomVariable):
    ...
    def sample(self, batch_size, num_particles):
        uniform = torch.FloatTensor(
            batch_size, num_particles
        ).uniform_() * self.choices.size(2)
        indices = torch.floor(uniform).unsqueeze(2)
        sampled_choices = torch.gather(
            self.choices, dim=2, index=indices.long()
        ).squeeze(-1)
        if self.is_cuda:
    ...
    def sample_reparameterized(self, batch_size, num_particles):
        warnings.warn(
            "Trying to sample_reparameterized from Choice(). Not supported! Sad."
        )
    ...
    def pdf(self, value, batch_size, num_particles):
        ...
        torch.zeros(batch_size, num_particles)
...
class Normal(RandomVariable):
    ...
    def __init__(self, mean, variance):
        ...
        self._multivariate_independent_normal = MultivariateIndependentNormal(
            mean=mean.unsqueeze(-1), variance=variance.unsqueeze(-1)
        )
...
class Pseudobernoulli(RandomVariable):
    ...
    def sample_reparameterized(self, batch_size, num_particles):
        return self._multivariate_independent_pseudobernoulli.sample_reparameterized(
            batch_size, num_particles
        ).squeeze(-1)
...
class Dirac(RandomVariable):
    ...
    def sample(self, batch_size, num_particles):
        if isinstance(self._value, Variable):
            return (
                Variable(torch.ones(batch_size, num_particles)) * self._value
            ).detach()
        else:
