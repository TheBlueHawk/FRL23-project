...
class AutoEncoder(nn.Module):
    ...
    def reconstruct_predict(
        ...
        given_obs = st.State(all_x=observation_states.all_x[:reconstruction_length])
        ...
        for t in range(reconstruction_length, num_timesteps):
            ...
            x = emission_state_random_variable.sample(batch_size, num_particles).all_x

            # This is usually done in the init-network
            # num_timesteps = 1 for one image
            # TODO: This is highly specific for VRNN
            phi_x = (
                self.encoding_network.phi_x(x.view(-1, nr_channels, w, h))
                .view(1, batch_size, num_particles, -1)
                .contiguous()
            )

            current_observation = st.State(
                all_phi_x=phi_x, x=x  # Just in case, not used in VRNN
            )
        ...
        all_obs = torch.zeros(
            num_timesteps, batch_size, num_particles, nr_channels, h, w
        )
        observation_states.unsequeeze_and_expand_all_(dim=2, size=num_particles)
        initial_state_random_variable = self.initial_network(observation_states)

        initial_state = initial_state_random_variable.sample(batch_size, num_particles)
        ...
        for t in range(num_timesteps):
            ...
            x = emission_state_random_variable.sample(batch_size, num_particles).all_x
            all_obs[t] = x.data
            averaged_obs[t] = summarize_function(x, inference_result.log_weight).data
