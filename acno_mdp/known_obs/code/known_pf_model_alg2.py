...
class DVRLPolicy(model.Policy):
    def __init__(
        ...
        if self.env == "cartpole":
            ...
            if (
                self.halved_acts
            ):  # know only 2 actions (dx, dy) affect the agent dynamics
                enc_actions -= 1
    ...
    def encode(
        ...
        all_phi_x = self.phi_x(observation_states.all_x.view(-1, *obs_dim)).view(
        ...
        for sample_i in range(batch_size):
            if self.env == "cartpole" and sum(actions[0, sample_i, :2] == 1) > 0:
                latent_state.z[sample_i, :, :] = Variable(
                    current_observation.all_x[sample_i, :, :].clone()
        ...
        emission_logpdf = torch.zeros(transition_logpdf.size()).to(
            transition_logpdf.device
        )
        return (
            latent_state,
            -encoding_logli,
