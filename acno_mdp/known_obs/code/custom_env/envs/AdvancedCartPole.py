...
class CustomCartPoleEnv(gym.Env):
    ...
    def step(self, combined_action):
        ...
        success = (
            theta < self.theta_success_threshold_radians
        ...
        return (
            np.array(self.obs).copy(),
            reward,
            done,
            {
                "action": action,
                "success": self.steps == self.max_episode_step,
                "comb action": combined_action,
                "observed": observe,
                "true state": np.array(self.state),
